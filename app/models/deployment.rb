class Deployment < ActiveRecord::Base
  ...
  def deploy_in_background!
    unless Rails.env.test?
      ...
      system("sh -c \"cd #{Rails.root} && /home/mithaichor/.rvm/gems/ruby-2.1.2/bin/rails runner -e #{Rails.env} ' deployment = Deployment.find(#{self.id}); deployment.prompt_config = #{self.prompt_config.inspect.gsub('"', '\"')} ; Webistrano::Deployer.new(deployment).invoke_task! ' >> #{Rails.root}/log/#{Rails.env}.log 2>&1\" &")
    end
  end
